...
const AulePrenotazione = ({ type, selectedDate, onDateChange, onRoomClick, selectedRoom, onOpenForm, onCloseForm, onOpenDetail, onCloseDetail }) => {
  ...
  // Modifica la funzione refreshPrenotazioni per aggiungere più dettagli nei log
  const refreshPrenotazioni = useCallback(async (formattedDate) => {
    console.log('=== INIZIO REFRESH PRENOTAZIONI ===');
    ...
    try {
      ...
      console.log('Dati prenotazioni recuperati:', JSON.stringify(data, null, 2));
      console.log('Eventuali errori prenotazioni:', error);
      ...
      console.log('=== FINE REFRESH PRENOTAZIONI ===');
    }
  }, []);

  useEffect(() => {
    const fetchAule = async () => {
      console.log('=== INIZIO FETCH AULE ===');
      setLoadingAule(true);
      try {
        const { data, error } = await supabase
          .from('aule')
        ...
        console.log('Dati aule recuperati:', JSON.stringify(data, null, 2));
        console.log('Eventuali errori aule:', error);
        ...
        console.log('=== FINE FETCH AULE ===');
      }
    };
    fetchAule();
  }, []);

  // Modifica la funzione fetchUtenti per aggiungere più log
  useEffect(() => {
    const fetchUtenti = async () => {
      try {
        console.log('=== INIZIO FETCH UTENTI ===');
        const { data, error } = await supabase
          .from('utenti')
          .select('*')
          .order('nome');
        
        console.log('Dati utenti recuperati:', data);
        console.log('Eventuali errori utenti:', error);
        
        if (error) throw error;
        setUtenti(data);
        console.log('=== FINE FETCH UTENTI ===');
      } catch (error) {
  ...
  const handlePrenotazione = async () => {
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        alert(`L'aula è già prenotata per le ${selectedTime} da ${existing[0].utenti.nome}`);
        return;
      }
      ...
      const { error: insertError } = await supabase
        ...
        .insert([{
          ...
          data: formattedDate,
          orario: selectedTime,
          id_utente: userName // This is now the user ID
        }]);
  ...
  const renderHeader = () => {
    return (
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Calendario Prenotazioni</h2>
        <div className="flex gap-2">
          {type === 'calendar' && (
            <>
              <button
                onClick={() => setShowExportModal(true)}
                className="inline-flex items-center justify-center px-3 py-1.5 text-sm text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              >
                <HiOutlineDocumentArrowDown className="w-5 h-5 mr-1" />
                Esporta Report
              </button>
              <button
                onClick={onOpenForm}
                className="inline-flex items-center justify-center w-8 h-8 text-white bg-blue-600 rounded-full hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              >
                +
              </button>
            </>
    ...
    <div className="max-w-[3000px] mx-auto">
      {loadingAule ? (
        ...
        <div className="space-y-6">
          {type === 'calendar' && (
            <div className="bg-gray-100 rounded-xl shadow-sm h-[580px] p-2 sm:p-4 border border-blue-400 relative">
              {renderHeader()}
          ...
          {type === 'form' && (
            <div className="bg-gray-100 p-3 sm:p-5 rounded-xl h-[400px] border border-blue-400 relative">
              <button
                onClick={onCloseForm}
                className="absolute right-4 top-4 text-gray-500 hover:text-red-600 text-2xl font-bold"
                title="Chiudi"
                style={{ zIndex: 10 }}
              >
                ×
          ...
          {type === 'list' && (
            <div className="bg-gray-100 rounded-xl shadow-sm h-[580px] border border-blue-400 relative">
              <h2 className="text-xl font-semibold text-gray-800 p-4 flex items-center justify-between">
                <span>🏫 Prenotazioni Aule</span>
                {typeof onOpenDetail === 'function' && (
                  <button
                    onClick={onOpenDetail}
                    className="absolute right-4 top-4 w-8 h-8 flex items-center justify-center rounded-full bg-blue-500 hover:bg-blue-600 text-white text-xl shadow focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all"
                            ...
                            <div key={aula.id} className="bg-gray-100 rounded-xl p-5">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 sm:gap-x-3 gap-y-2 sm:gap-y-3 max-h-[350px] overflow-y-auto">
                                {prenotazioniOrari.map(({ orario, prenotazione }) => {
                                  ...
                                  const canDelete = prenotazione && canDeleteBooking(prenotazione.utenti.nome);
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="flex justify-between items-center">
                                        ...
                                        {isOccupato && canDelete && (
                                          <button
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleAnnullaPrenotazione(prenotazione.id, prenotazione.utenti.nome);
                                            }}
                                      ...
                                      <div className="text-xs">
                                        {isOccupato ? (
                                          <span className="font-medium text-white truncate">
                                            Prenotato da: {prenotazione.utenti.nome} ({prenotazione.utenti.qualifica})
                                          </span>
                                        ) : (
          ...
          {type === 'detail' && (
            <div className="bg-gray-100 rounded-xl shadow-sm h-[400px] p-2 sm:p-4 border border-blue-400 relative">
              {typeof onCloseDetail === 'function' && (
                <button
                              ...
                              dateBookings.map((booking, index) => (
                                <tr key={`${date}-${index}`} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                  ...
                                  <td className="w-5/12 px-3 py-1.5 whitespace-nowrap text-sm text-gray-800 font-medium">
                                    {booking.utenti ? `${booking.utenti.nome} (${booking.utenti.qualifica})` : '—'}
                                  </td>
                                </tr>
                              ))
                            ))
